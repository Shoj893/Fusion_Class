...
def student_dashboard():
    ...
    for i, sub_node in enumerate(subjects):
        ...
        def unenroll_button():
                # Use a unique key for each subject card button to avoid Streamlit duplicate element IDs.
...
def student_screen():
    ...
    with c2:
        if st.button("Go back to Home", type="secondary", key="loginbackbtn", shortcut="command+backspace"):
        ...
        with st.spinner("AI is scanning..,"):
            ...
            if num_faces == 0:
                ...
                if detected:
                    ...
                    st.info("Face not recognise! You might be a new student!")
                    show_registration = True

    if show_registration:
        with st.container(border=True):
            st.header("Register new Profile")
            new_name = st.text_input("Enter your name", placeholder="Eg. Shobhit Jaiswal")
            ...
            try:
                audio_data = st.audio_input("Record a short phrase like I am present, My name is Shobhit.")
